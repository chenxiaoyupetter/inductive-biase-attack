...
#import cnns_method
from utils import BASE_ADV_PATH, ROOT_PATH,BASE_ADV_PATH1
import our_method
...
def arg_parse():
    ...
    args.opt_path = os.path.join(BASE_ADV_PATH, 'model_{}-method_{}-{}{}'.format(args.model_name, args.attack, args.filename_prefix,args.boundary))

    args.opt_path1 = os.path.join(BASE_ADV_PATH1, 'model_{}-method_{}-{}'.format(args.model_name, args.attack, args.filename_prefix))

    if not os.path.exists(args.opt_path):
        os.makedirs(args.opt_path)
    return args
    if not os.path.exists(args.opt_path1):
        os.makedirs(args.opt_path1)
    return args
...
if __name__ == '__main__':
    ...
    # Attack getattr() 函数用于返回一个对象属性值。
    attack_method = getattr(our_method, args.attack)(args.model_name,args.boundary)
    #Attack I 函数用于返回一个对象属性值
    ...
    for batch_idx, batch_data in enumerate(data_loader):
        ...
        #batch_x 原始动量
        adv_inps, loss_info,adv_perts = attack_method(batch_x, batch_y)
        #adv_inps, loss_info = attack_method(batch_x, batch_y)

        attack_method._save_images(adv_inps, batch_name, args.opt_path)
        #attack_method._save_images(adv_perts, batch_name, args.opt_path1)
