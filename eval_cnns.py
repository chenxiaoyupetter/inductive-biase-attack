...
def load_images(input_dir, batch_shape):
    ...
    for filepath in tf.gfile.Glob(os.path.join(input_dir, '*')):
        with tf.gfile.Open(filepath, 'rb') as f:
            image = imread(f, pilmode='RGB')
            x = 299/224
            image = cv2.resize(image, None, None, fx=x, fy=x, interpolation=cv2.INTER_LINEAR).astype(np.float) / 255.0
...
if __name__ == '__main__':
    f2l = load_labels('./dev_data/ps-0.3.csv')
    input_dir = './outputs/0.8* cost4 + 0.1-0.6(cost5+0.1) +0.1-0.6*cost3/model_visformer_small-method_OurAlgorithm-paper_results1.0'
